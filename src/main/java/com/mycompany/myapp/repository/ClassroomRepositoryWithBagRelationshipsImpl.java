...
public class ClassroomRepositoryWithBagRelationshipsImpl implements ClassroomRepositoryWithBagRelationships {
    ...
    public Optional<Classroom> fetchBagRelationships(Optional<Classroom> classroom) {
        return classroom.map(this::fetchStudentIDS);
    }
    ...
    public List<Classroom> fetchBagRelationships(List<Classroom> classrooms) {
        return Optional.of(classrooms).map(this::fetchStudentIDS).orElse(Collections.emptyList());
    }

    Classroom fetchStudentIDS(Classroom result) {
        return entityManager
            .createQuery(
                "select classroom from Classroom classroom left join fetch classroom.studentIDS where classroom is :classroom",
                Classroom.class
            )
    ...
    List<Classroom> fetchStudentIDS(List<Classroom> classrooms) {
        return entityManager
            .createQuery(
                "select distinct classroom from Classroom classroom left join fetch classroom.studentIDS where classroom in :classrooms",
                Classroom.class
            )
